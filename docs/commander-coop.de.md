...
3. Wähle im ausgeschalteten Zustand mit Links/Rechts die lokale private
   IPv4-Adresse. Loopback 127.0.0.1 wird nur für Tests auf demselben Rechner
   angeboten. Falls keine LAN-Adresse erscheint, prüfe die Netzwerkverbindung des
   Geräts; die Erkennung führt weder DNS- noch Internetabfragen durch.
4. Wähle einen Port, falls der Standardport 8765 belegt ist. Aktiviere den Dienst
   mit Enter.
5. Öffne in jedem Browser der Besatzung die tatsächlich angezeigte URL. Erlaube
   eingehende Verbindungen in der Host-Firewall bei Bedarf ausschließlich aus dem
   vertrauenswürdigen LAN.
6. Gib den lokalen Kopplungscode ein: drei Ziffern gefolgt von drei
...
7. Fordere nach der Kopplung im Browser eine Station an. Der Host genehmigt oder
...
Lokales Deaktivieren oder das Beenden des Prozesses stoppt den Listener und
widerruft Zugangsdaten. Erfolgreiches Laden/Zurücksetzen ersetzt die
